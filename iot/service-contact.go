...
// func (sc *ServiceContact) Refresh( now uint32) {
// 	// in testing reset contact expiry
// 	sc.contact.contactExpires = now + 60*20
// }
...
func (sc *ServiceContact) startReadTheWriterPipe() {
	go func() {
		for {
			select {
			// case <-sc.ex.??:
			// 	fmt.Println("http subdomain handler executive closed")
			// 	return
			case <-sc.contact.ClosedChannel:
