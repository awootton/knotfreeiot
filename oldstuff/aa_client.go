...
func aaLightSwitch(mySubChan string, ourSwitch string) {

	if aaExpectedConnections > 10 { // 60 sec * 30 = 1800 sec = 30 min
		time.Sleep(time.Duration(rand.Intn(60)) * time.Second * 30)
	}
	if aaExpectedConnections == 1 {
...
// add 127.0.0.1 knotfreeserver to /etc/hosts
func aaLightController(id string, target string) {

	if aaExpectedConnections > 10 {
		time.Sleep(time.Duration(rand.Intn(60)) * time.Second * 30)
	}
...
func init() {
	...
	reporting.NewGenericEventAccumulator(aaclientReporter)
}
